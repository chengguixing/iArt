 ...
 * 设置文件保存位置
 */
void CClientDlg::OnButtonSetFilePath() 
{
 ...
 * 接收目录包括根目录
 */
BOOL CClientDlg::RecvDirString(int nDataLen)
{
	BOOL reVal = TRUE;		//返回值
 ...
 */
int CClientDlg::RecvFileData(SOCKET s, unsigned short ID)
{
	int nErrCode;	//返回错误
	hdr header;		//包头
 ...
 * 保存临时文件
 */
 void CClientDlg::SaveFile(u_short nIndexThread, const char* pBuf, u_long ulLen, u_long ulPos)
{
	FILE *stream = NULL;
 ...
 * 合并文件
 */
void CClientDlg::AssemblyFile(void)
{
	//文件指针	
 ...
 * 合并临时文件
 */
u_long  CClientDlg::AssembleLine(FILE* destFile, int startPos, FILE* resoFile)
{
	//获取读文件的长度
 ...
 * 接收文件大小
 */
void CClientDlg::RecvFileSize(unsigned long ulFileLen)
{
 ...
 * 准备创建文件下载线程
 */
void CClientDlg::PrepareFileDownload(void)
{
	DWORD dwThreadID;
 ...
 * 创建套接字
 */
BOOL CClientDlg::CreateSocket(SOCKET &s)
{
		//创建套接字
 ...
 * 创建文件下载线程
 */
DWORD CClientDlg::CreateDownloadFileThread(void *pParam)
{
	CClientDlg* pClient = (CClientDlg*)pParam;
 ...
 *	下载文件线程
 */
DWORD CClientDlg::DownloadFileThread(void* pParam)
{
	...
	CClientDlg *pClient = pFileDownload->ptr;		//指针
 ...
 * 显示目录
 */
void CClientDlg::PaintTree(HTREEITEM hTreeItem, list<string> &listSubDir)
{
 ...
 * 得到文件路径
 */
void CClientDlg::GetFilePath(HTREEITEM hTreeItem, string &strPath)
{
	string		strTemp;//临时变量
 ...
 * 浏览目录
 */
void CClientDlg::OnItemexpandingTree(NMHDR* pNMHDR, LRESULT* pResult) 
{
 ...
 * 显示菜单
 */
void CClientDlg::OnDblclkTree(NMHDR* pNMHDR, LRESULT* pResult) 
{
 ...
 * 设置控件状态
 */
void CClientDlg::EnableSomeControl(BOOL bEnable)
{
	if (!bEnable)
	{
 ...
 * 隐藏进度条控件
 */
void CClientDlg::ResetCurFileDL(void)
{
	m_ulCurTotal = 0;
 ...
 * 响应菜单操作
 */
void CClientDlg::OnFileDownload() 
{
	//判断是否设置了保存文件的路径
