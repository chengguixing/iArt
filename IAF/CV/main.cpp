/*
	条件变量相关函数						说明   
	InitializeConditionVariable				初始化一个条件变量
	SleepConditionVariableCS				以原子操作的方式同时执行以下两个操作：在指定条件变量上等待、释放特定的critical section。
	SleepConditionVariableSRW				以原子操作的方式同时执行以下两个操作：在指定条件变量上等待并释放某个SRW锁。
	WakeAllConditionVariable				唤醒所有在同一个条件变量上等待的多个线程。
...
DWORD WINAPI ProducerThreadProc(PVOID);
DWORD WINAPI ConsumerThreadProc(PVOID);
