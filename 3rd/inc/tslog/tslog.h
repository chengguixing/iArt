...
//#define TSLOG_GROUP "Thunder" 

/********************************************************************************************************
	tslog大小不变!	定义版本,每次更改加1,版本号用来判断预留空间的使用情况
********************************************************************************************************/
#define TSLOG_VERSION	0x00025093
//兼容LOG4C
	...
	*/
	...
	inline  CTSLog& 		operator << (const char * _Val) throw()
	{
		...
		if(m_dwUsedSize < m_dwMaxLogDataSize)
		{
			...
			__try
			{
				nLen = MultiByteToWideChar(GetStringCodePage(_Val)  ,0,_Val,-1,NULL,0);
			}
			__except(EXCEPTION_EXECUTE_HANDLER)
			{	
				nLen = 0;
			...
			if(0 < nLen)
			{
				if(MAX_USERDATA_SIZE + 1 < nLen)
				{
					...
					nLen = MultiByteToWideChar(GetStringCodePage(_Val),0,_Val, -1, szBufferW,nLen);
				}
				else
					nLen = MultiByteToWideChar(GetStringCodePage(_Val),0,_Val, -1, szBufferW,nLen);
				if(0 < nLen)
	...
	inline  CTSLog& 		operator << (const wchar_t * _Val) throw()
	{
		...
		if(m_dwUsedSize < m_dwMaxLogDataSize)
		{
			...
			__try
			{
				nLen = WideCharToMultiByte(CP_ACP, 0, _Val, -1,NULL, 0 ,NULL, NULL);
			}
			__except(EXCEPTION_EXECUTE_HANDLER)
			{
				nLen = 0;
			...
			if(0 < nLen)
			{
				if(MAX_USERDATA_SIZE + 1 < nLen)
				{
					...
					_Val = szLogDataW;
					nLen = MAX_USERDATA_SIZE + 1;
					nLen = WideCharToMultiByte(CP_ACP, 0, _Val, -1, szBufferA, nLen,NULL,NULL);
				}
				else
					nLen = WideCharToMultiByte(CP_ACP, 0, _Val, -1, szBufferA, nLen,NULL,NULL);
				if(0 < nLen)
...
public:
	...
	{
		if(s_release.bIsModuleUnloading)
		{
		...
		return s_release.bIsModuleUnloading;
	}
...
private:
friend class CEnterLeaveFunCallBack;
friend class CTSGHeapVector;
inline	CTSLog(){};
inline	~CTSLog(){};
private:
#pragma pack(push, 1)
...
#pragma pack(push, 1)
	...
	friend class CTSLog;
	private:
		volatile LONG  bIsModuleUnloading;
		HMODULE m_hLang;
	public:
		CRelease(INT):bIsModuleUnloading(FALSE), m_hLang(NULL)
		{
		...
		{
			::InterlockedExchangeAdd(&bIsModuleUnloading,1L);
...
BOOL CTSLog::UnicodeToMultiByte(const std::basic_string<WCHAR> &strSrc, std::string &strDst)
{
	...
	if(0 == nSrcUnicodeLen)
		return TRUE;
	static INT nLen = 0; 
	nLen = WideCharToMultiByte(CP_ACP, 0, strSrc.c_str(), -1,NULL, 0 ,NULL, NULL);
	CHAR szBuf[MAX_USERDATA_SIZE + 1] = {0};
	LPSTR pszBuf = szBuf;
	if(MAX_USERDATA_SIZE < nLen)
		nLen = MAX_USERDATA_SIZE;
	INT nResult = 0;
	nResult = WideCharToMultiByte(CP_ACP, 0, strSrc.c_str(), -1, pszBuf, nLen,NULL,NULL);
	if( nResult <= 0)
		return FALSE;
 	if(pszBuf)
		strDst = pszBuf;
...
BOOL CTSLog::MultiByteToUnicode(const std::string& strSrc, std::basic_string<WCHAR> &strDst)
{
	...
	if(0 == nSrcAnsiLen)
		return TRUE;
	INT nLen = 0;
	nLen = MultiByteToWideChar(GetStringCodePage(strSrc.c_str()),0,strSrc.c_str(),-1,NULL,0);
	if(MAX_USERDATA_SIZE < nLen)
		nLen = MAX_USERDATA_SIZE;
	WCHAR szBuf[MAX_USERDATA_SIZE + 1] = {0};
	LPWSTR pszBuf = szBuf;
	INT nResult = 0;
	nResult = MultiByteToWideChar(GetStringCodePage(strSrc.c_str()),0,strSrc.c_str(), -1, pszBuf,nLen);
	if(nResult <= 0 )
		return FALSE;	
 	if(pszBuf)
		strDst = pszBuf;
...
CTSLog & CTSLog::GetInstance(LPCTSTR pszCallFunName, LPCVOID pvThis)
{
	...
	TlsSetValue(s_dwTlsIndex, (LPVOID)pszCallFunName);
	TlsSetValue(s_dwTlsIndex_this, (LPVOID)pvThis);
	s_release.bIsModuleUnloading = FALSE; //禁止 Release 优化,这一行非写不可,2当模块退出时，不检测日志是否被更改
	if( bExist )
	{
...
//	size_t  nFinalFlagLen = _tcslen(_T("\t\r\n"));
	...
	char* lpLuaStack = NULL;
	if (level == LEVEL_WARN || level == LEVEL_ERROR || level == LEVEL_FATAL)
	{
		...
		if (lpGetLuaStack != NULL)
		{
			lpLuaStack = (char*)::malloc(1024 * 64);
			lpGetLuaStack(NULL, lpLuaStack, 1024 * 64);
		}
	}
	...
	{
	case LEVEL_WARN:	
		ret = MessageBox(NULL,pWholeMsg,TEXT("Warn---Yes查看lua调用栈,No进行调试,Cancel直接忽略"),MB_ICONWARNING | MB_YESNOCANCEL | MB_DEFBUTTON3);
		break;
	case LEVEL_ERROR:	
		ret = MessageBox(NULL,pWholeMsg,TEXT("Error---Yes查看lua调用栈,No进行调试,Cancel直接忽略"),MB_ICONERROR | MB_YESNOCANCEL | MB_DEFBUTTON3);
		break;
	case LEVEL_FATAL:	
		ret = MessageBox(NULL,pWholeMsg,TEXT("Fatal---Yes查看lua调用栈,No进行调试,Cancel直接忽略"),MB_ICONERROR | MB_YESNOCANCEL | MB_DEFBUTTON3);
		break;
	default:
		;
	}
	...
	if (ret == IDYES)
	{
		if (lpLuaStack != NULL)
		{
			::MessageBoxA(NULL, lpLuaStack, "LUA调用栈", MB_ICONINFORMATION | MB_OK);
		}
	}
	else if (ret == IDNO)
	{
...
{
	UINT code_page = CP_ACP;//default value
	if(NULL == szData)
		return code_page;
	...
	__try
	{
		...
		if(SUCCEEDED(hr) && pMLang)
		{
			...
			{
				confidence = dei[0].nConfidence;
				if (confidence > 50)
				{
					code_page=dei[0].nCodePage;
				}
			}
			pMLang->Release();
		}
...
{	
	...
	INT nLen = 0;
	nLen = WideCharToMultiByte(CP_ACP, 0, pszFileLogMsg, -1,NULL, 0 ,NULL, NULL);
	if(0 >= nLen)
		return;
	else if(MAX_USERDATA_SIZE + MAX_PRIVATEDATA_SIZE  < nLen)
		return;
	dwLen = WideCharToMultiByte(CP_ACP, 0, pszFileLogMsg, -1, szBufferA, nLen,NULL,NULL);	
	if(0 >= dwLen)
		return;
	dwLen--;
...
#define MAX_ALL_HEX_DUMP_SIZE MAX_HEX_DUMP_COUNT * MAX_HEX_DUMP_SIZE
	...
	if(NULL == pNext)
	{
		...
		OutputLog(level, _T("TSLOG MEMORY HEXDUMP Begin [%p], Total Lenght : [%lu] bytes, Prompt : %s") , ap);
	}
...
#ifdef W2A	
				dwRet = _snprintf(tslog_szRegValue,MAX_USERDATA_SIZE , " = 0x%08Ix(L\"%s\")", tslog_pRegValueW , W2A(tslog_pRegValueW));
#else			
				dwRet = _snprintf(tslog_szRegValue,MAX_USERDATA_SIZE , " = 0x%08Ix(L\"%S\"?)", tslog_pRegValueW , tslog_pRegValueW);
#endif
			break;
				...
				case VT_BSTR:
					if(pVar->bstrVal)
					{
						if(MAX_USERDATA_SIZE/2 < wcslen(pVar->bstrVal))
							...
							dwRet = _snprintf(tslog_szRegValue, MAX_USERDATA_SIZE," = {L\"%s\" VT_BSTR}", OLE2A(pVar->bstrVal));
					}
					else
...
#if (defined(UNICODE) || defined(_UNICODE))
		INT nLen = 0;
		nLen = WideCharToMultiByte(CP_ACP, 0, szData, -1,NULL, 0 ,NULL, NULL);
		dwAllLen = WideCharToMultiByte(CP_ACP, 0, szData, -1, szBufferA, sizeof(CHAR) * nLen,NULL,NULL);		
		dwAllLen--;
